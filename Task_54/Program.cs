...
matrix              = RowDescendingMatrixArrange( matrix );
             Console. WriteLine                 ( "Now after ordering the rows in descending order:");
                      PrintMatrixInt            ( matrix, arrange );        /**/
...
int[,] RowDescendingMatrixArrange(int[,] mtrx){
    int [,] mssv = new int[mtrx.GetLength(0), mtrx.GetLength(1)];
    ...
    for(int i = 0; i < mssv.GetLength(0); i++){
        for(int j = 0; j < mssv.GetLength(1); j++){           
            for(int jSx = j + 1; jSx < mssv.GetLength(1); jSx++){
                if( mssv[i, j] < mssv[i, jSx] ){
                    int minValue    = mssv[i, j];
                    mssv[i, j]      = mssv[i, jSx];
                    mssv[i, jSx]    = minValue;
                }
            }
